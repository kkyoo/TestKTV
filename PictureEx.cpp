	...
	UINT *pDisposal)
{
	try
	{
		...
		*pDisposal = 0;
		...
		if(nStart<m_nDataSize && (nStart+nBlockLen)<m_nDataSize)
		{
...
void CPictureEx::OnPaint() 
{
	...
	if(m_bResizePic)
	{
		...
		{  
			...
			int x=0,y=0;
			if(rcClient.Width()> nPaintWidth)
				x = (rcClient.Width() - nPaintWidth) / 2;
			if(rcClient.Height() >nPaintHeight)
				y = (rcClient.Height() - nPaintHeight) / 2;

			::StretchBlt(dc.m_hDC, x, y, nPaintWidth,nPaintHeight, m_hMemDC, 0,0,m_PictureSize.cx, m_PictureSize.cy,SRCCOPY);   
		}  
		else  
		{  
	...
	}else
	{
		...
		if (nPaintWidth > 0)
		{
			...
			if(nPaintWidth>=m_PictureSize.cx && nPaintHeight>=m_PictureSize.cy)
			{
				int x = (rcClient.Width() - m_PictureSize.cx) / 2;
				int y = (rcClient.Height() - m_PictureSize.cy) / 2;

				::BitBlt(dc.m_hDC, x, y, nPaintWidth, nPaintHeight,m_hMemDC,0, 0, SRCCOPY);
			}
			else
			{
				int nCx = m_PictureSize.cx;
				int nCy = m_PictureSize.cy;

				if(nPaintWidth<nCx || nPaintHeight<nCy)
				{
					double x=0.0,y=0.0;
					x=(double)nCx/nPaintWidth;
					y=(double)nCy/nPaintHeight;

					if(x>y)
					{
						double dFlag = (double)nPaintWidth / nCx ;
						nCy = m_PictureSize.cy * dFlag;

						nCx = nPaintWidth;
					}
					else
					{
						double dFlag = (double)nPaintHeight / nCy;
						nCx = m_PictureSize.cx * dFlag;
				...
				int x = (rcClient.Width() - nPaintWidth) / 2;
				int y = (rcClient.Height() - nPaintHeight) / 2;

				dc.SetStretchBltMode(HALFTONE);
				::StretchBlt(dc.m_hDC, x, y, nPaintWidth, nPaintHeight,	
		...
		else
		{
			...
			::BitBlt(dc.m_hDC, x, y, m_PictureSize.cx, m_PictureSize.cy,
...
void CPictureEx::DrawFrame(CDC* pDC, UINT nFrame)
{
	if(nFrame<0 || pDC==NULL)
		return;

	if(IsAnimatedGIF())
	{
		if(nFrame>=GetFrameCount())
			return;
		...
		for(int i=0;i<=nFrame;i++)
		{
			...
			if (m_arrFrames[i].m_pPicture)
			{
				...
				long hmWidth;
				long hmHeight;
				m_arrFrames[i].m_pPicture->get_Width(&hmWidth);
				m_arrFrames[i].m_pPicture->get_Height(&hmHeight);

				m_arrFrames[i].m_pPicture->Render(pDC->GetSafeHdc(), 
	...
	else
	{
		if (m_pPicture)
		{
			...
			m_pPicture->get_Width(&hmWidth);
			m_pPicture->get_Height(&hmHeight);
			m_pPicture->Render(pDC->GetSafeHdc(), 0, 0, m_PictureSize.cx, m_PictureSize.cy, 0, hmHeight, hmWidth, -hmHeight, NULL);
		};
	}
}
void CPictureEx::ReSizePic(CSize sz)
{
	int width,height;
	...
	if(width>sz.cx)
	{
		height = height*sz.cx/width;
		width = sz.cx;
	}
	if(height>sz.cy)
	{
		width = width*sz.cy/height;
		height =sz.cy; 
	}
	CRect rect,rt;
	GetClientRect( &rect);

...
#define WM_DRAW_GOOD     WM_USER+4509
...
void CListGiftDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_GIFT, m_listGift);
}
...
BEGIN_MESSAGE_MAP(CListGiftDlg, CDialog)
	...
	ON_MESSAGE(WM_DRAW_GOOD,OnLoadGiftImgComplete)
END_MESSAGE_MAP()
...
BOOL CListGiftDlg::OnInitDialog()
{
	...
	LONG style = GetWindowLong(m_listGift, GWL_STYLE);
	style|= LVS_AUTOARRANGE;
	SetWindowLong(m_listGift, GWL_STYLE, style);

	//m_listGift.SubclassDlgItem(IDC_LIST_GIFT, this);     //图片直接的间隔距离会出错,DoDataExchange继承控件即可
	m_listGift.SetIconSpacing(50, 50);

	m_imglstGift.DeleteImageList();
	BOOL bResult = m_imglstGift.Create(GIFT_PIC_W, GIFT_PIC_H, ILC_COLOR16| ILC_MASK, 20, 10);
	ASSERT( bResult );
	m_listGift.SetImageList( &m_imglstGift, LVSIL_NORMAL);
...
void CListGiftDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if(m_listGift.GetSafeHwnd())
	{
		CRect rtClient,rtListGift;
		GetClientRect(&rtClient);
		m_listGift.MoveWindow(&rtClient);
	}
}
...
void CListGiftDlg::OnNMClickListGift(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		pGiftData pData = ( pGiftData )m_listGift.GetItemData( pNMListView->iItem );
		ASSERT( pData );
		CString s;
		m_curSelGift = *pData;
...
{
	for(int i=0; i<m_listGift.GetItemCount(); i++)
	{
		pGiftData pData = ( pGiftData )m_listGift.GetItemData(i);
		ASSERT(pData);
		if ( pData)
			delete pData;
		pData = NULL; 
	}
	m_listGift.DeleteAllItems();
}
			...
			if ( hGlobal )
			{
				if ( ( pvData = GlobalLock(hGlobal) ) != NULL)	// 锁定分配内存块
				{
					...
					if( SUCCEEDED(bResult)  && pPic)
					{
						...
						index = m_imglstGift.Add(CBitmap::FromHandle(hbm), RGB(255,255,255));
						index = m_listGift.InsertItem(m_listGift.GetItemCount(), "", index);
						...
						m_listGift.SetItemData( index, (DWORD_PTR)pData );
...
bool CListGiftDlg::LoadGiftImg()
{
	...
	PostMessage(WM_DRAW_GOOD,NULL,NULL);
	return bSucceeded;
}
